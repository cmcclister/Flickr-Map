...
const add_marker = item => {
    ...
    let postHtml = `<div class="card"><div class="user"><a href="https://flickr.com/photos/${item.owner}/${item.id}" target="_blank"><img src="${postSrc}"></a></div><div class="post"><b>${item.title}</b><br />taken ${item.datetaken.split(' ')[0]}<br />by <a href="https://flickr.com/people/${item.owner}" target="_blank">"${item.ownername}</a><br />with ${item.views}views<div class="desc">${item.description._content}</div><div class="tags">${item.tags}</div></div></div>`;
